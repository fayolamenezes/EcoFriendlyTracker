...
const App = () => {
  ...
  useEffect(() => {
    ...
    const savedCO2Reduction = savedActions.reduce((acc, action) => acc + action.totalCO2Reduction, 0);
  ...
  const handleAddAction = (newAction) => {
    ...
    setTotalCO2Reduction(updatedActions.reduce((acc, action) => acc + action.totalCO2Reduction, 0));
  ...
  return (
    <div className="App">
      ...
      <ActionList actions={actions} handleAddAction={handleAddAction} />
      <ImpactSummary totalCO2Reduction={totalCO2Reduction} actions={actions} handleClearActions={handleClearActions} />
    </div>
  );
};
